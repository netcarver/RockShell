...
class PwSetup extends Command
{
  ...
  public function handle()
  {
    ...
    $dst = $this->app->pwRootPath() . ".gitignore";
    $write = true;
    if (is_file($dst)) {
    ...
    if ($split and $this->confirm("Split config into config.php and config-local.php?", true)) {
      ...
      $dst = $this->app->pwRootPath() . "site/config-local.php";
      $write = true;
      if (is_file($dst)) {
        if (!$this->confirm("$dst exists - overwrite it?", false)) {
          $this->write("Aborting...");
      ...
      $dst = $this->app->pwRootPath() . "site/config.php";
  ...
  {
    $config = $this->app->pwRootPath() . "site/config.php";
